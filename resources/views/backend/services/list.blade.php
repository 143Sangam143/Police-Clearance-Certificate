<div class="min-h-screen !bg-fixed !bg-no-repeat !bg-center !bg-cover pt-[8rem] pb-[3rem] space-y-[1.5rem]" style="background-image:url('/frontend/images/services/services-bg.png');">
    ...
    <div class="grid grid-cols-1 min-[350px]:grid-cols-2 xl:grid-cols-3 items-center justify-center px-[5%] 2xl:px-[10vw] gap-y-[1.5rem] scroll-smooth">
                        ...
                        <div class="py-[1rem] font-medium font-mono text-center space-y-[.8rem]">
                            ...
                            <h3 class="hidden text-[1.2rem] sm:text-[1.5rem] text-[#222] text-start px-2">
                                <div class="relative z-0 hidden">
                                    <input type="text" id="floating_category" class="block capitalize text-center px-0 w-full text-[1.2rem] sm:text-[1.5rem] text-[#222] bg-transparent border-2 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-gray-300 focus:border-b-blue-600 peer" name="category" minlength="2" maxlength="30" required value="{{$services}}" />
        ...
        @else
            @if(\Route::current()->getName() == 'backend.services.list.update')
                <div class="group [perspective:1000px]">
                    <div class="relative transition-all duration-500 [transform-style:preserve-3d] :[transform:rotateY(180deg)]">
                        <form class="w-[8rem] sm:w-[13.8rem] md:w-[19.8rem] !bg-center !bg-cover overflow-hidden rounded-[.7rem] bg-white border-y-4 border-[#222] ease-linear duration-300 mx-auto" action="{{ route('backend.services.list.update-confirm', $lists->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="picture overflow-hidden w-[8rem] sm:w-[13.8rem] md:w-[19.8rem] h-[6rem] sm:h-[13.8rem] md:h-[19.8rem] ">
                                <img id='img' class="block hidden w-full h-full object-contain">
                                <div class="max-sm:p-[.5rem] p-[2rem] w-full h-full" id="imgUploader">
            ...
            @else
                @foreach($lists as $list)
                    <div class="space-y-[.5rem] items-center flex flex-col">
                        <a href="{{ route('backend.services.list.details') }}" class="hover:scale-[1.02] group [perspective:1000px]">
                            <div class="relative transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                                <div class="w-[8rem] sm:w-[13.8rem] md:w-[19.8rem] !bg-center !bg-cover overflow-hidden rounded-[.7rem] bg-gradient-to-r from-[#f9f9f9]  via-[#ececec] to-[#f8f8f8] border-y-4 border-[#222] ease-linear duration-300 mx-auto">
                                    <div class="picture overflow-hidden w-[8rem] sm:w-[13.8rem] md:w-[19.8rem] h-[6rem] sm:h-[13.8rem] md:h-[19.8rem]">
                                        <img src="/images/backend/services/list/{{ $list->image }}" alt="" class="block w-full h-full object-contain">
                                    </div>
                                    <hr class="z-10 h-[.1rem] bg-[#ccc]">
                        ...
                        <div class="flex max-md:flex-col justify-between max-md:space-y-[.2rem] w-[8rem] sm:w-[13.8rem] md:w-[19.8rem]">
                            <a href="{{ route('backend.services.list.delete', $list->id) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onclick="return confirm('Are you sure about this?')">
                                    Delete
                            </a>
                            <a  href="{{ route('backend.services.list.update', [$list->id, $list->category]) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Update
                            </a>
                        </div>
                    </div>
            ...
            <div class="group [perspective:1000px] hidden" id="items">
                <div class="relative transition-all duration-500 [transform-style:preserve-3d] :[transform:rotateY(180deg)]">
                    <form class="w-[8rem] sm:w-[13.8rem] md:w-[19.8rem] !bg-center !bg-cover overflow-hidden rounded-[.7rem] bg-white border-y-4 border-[#222] ease-linear duration-300 mx-auto" action="{{ route('backend.services.list.add') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="picture overflow-hidden w-[8rem] sm:w-[13.8rem] md:w-[19.8rem] h-[6rem] sm:h-[13.8rem] md:h-[19.8rem] ">
                            <img id='img' class="block hidden w-full h-full object-contain">
                            <div class="max-sm:p-[.5rem] p-[2rem] w-full h-full" id="imgUploader">
